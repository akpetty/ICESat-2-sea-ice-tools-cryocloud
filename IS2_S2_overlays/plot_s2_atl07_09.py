""" plot_s2_comparison.py
    ...
    example: 
      run as: python plot_s2_atl07_09.py -b 'gt1l' -e 'P5'
...
def pick_profile(example):
  ...
  if example=='P1':
    ...
    # ATLAS DATA
    # -1 indicates we're using the latest version of the data (i.e. the higher number)
    ATL07_dataFile = glob(data_path+relStr+'/ATL07-01/data/ATL07-01_20190726*441*'+relStr[-3:]+'_'+version+'.h5')[-1]
    ATL10_dataFile = glob(data_path+relStr+'/ATL10-01/data/ATL10-01_20190726*441*'+relStr[-3:]+'_'+version+'.h5')[-1]
    ...
    Slim = 60
    Nlim = 88
    bufferRatio=100 # Bigger number smaler width (try 100 for zoom in)
    ...
    # ATLAS DATA
    # -1 indicates we're using the latest version of the data (i.e. the higher number)
    ATL07_dataFile = glob(data_path+relStr+'/ATL07-01/data/ATL07-01_20190726*1301*'+relStr[-3:]+'_'+version+'.h5')[-1]
    ATL10_dataFile = glob(data_path+relStr+'/ATL10-01/data/ATL10-01_20190726*1301*'+relStr[-3:]+'_'+version+'.h5')[-1]
    ...
    # ATLAS DATA
    # -1 indicates we're using the latest version of the data (i.e. the higher number)
    ATL07_dataFile = glob(data_path+relStr+'/ATL07-01/data/ATL07-01_20190726*1294*'+relStr[-3:]+'_'+version+'.h5')[-1]
    ATL10_dataFile = glob(data_path+relStr+'/ATL10-01/data/ATL10-01_20190726*1294*'+relStr[-3:]+'_'+version+'.h5')[-1]
  ...
  return geoTIFF, imageFile, ATL07file, ATL10file, Slim, Nlim, bufferRatio, date
...
def parse():
    ...
    parser.add_argument('-e', action='store', default='', dest='example',
                        ...
                        type=str)
    parser.add_argument('-b', action='store', default='', dest='beam_num',
                        help='beam num (e.g. 1, 2, 3, 4, 5, 6)',
                        type=str)
...
def get_geoTIFF_info(geoTIFF, plot=False):
  ...
  return surfReflectance, geoTIFF_extent, out_proj, geoTIFF_geo_trans, band
...
def plot_height_freeboard(sub_dF07, sub_dF10):
  ...
  #plt.tight_layout()
  ### Save plot to file
  plt.savefig(figPath+'s2-extra/diagnostic_'+date+beamStr+relStr+str(Slim)+str(Nlim)+example+'.png', dpi=300)
...
def plot_diagnostic_boxplot(sub_dF07):
  ...
  plt.savefig(figPath+'s2-extra/diagnostic_boxplot_'+date+beamStr+relStr+str(Slim)+str(Nlim)+example+'.png', dpi=300)
...
#s2_data_path = '/sea_ice_pso/aapetty/raw_data/S2/'
s2_data_path = '/cooler/sea_ice_pso/aapetty/raw_data/S2/'
#s2_data_path = '/cooler/scratch1/mbagnard/S2_extra_imagery/20190726_T09XWJ_T09XWH_T09XVH_T10XDQ_RGT441_DT22_AR/'
figPath='../output/'+relStr+'/profiles/atl07_09_s2/'
...
if inps.example is None:
    ...
    sys.exit(1)
if inps.beam_num is None:
    print('Missing beam. Use option -b')
    sys.exit(1)

example = inps.example
beam_num = inps.beam_num

print(example, beam_num)

################### Read S-2/ICESat-2 data for given example profile ########################################
print('Read in S-2/ICESat-2 data...')
geoTIFF, imageFile, ATL07_dataFile, ATL10_dataFile, Slim, Nlim, bufferRatio, date = pick_profile(example)

### Read geoTIFF file and print spatial extent information
surfReflectance, geoTIFF_extent, out_proj, geoTIFF_geo_trans, band = get_geoTIFF_info(geoTIFF)

### Read ATL07 file into pandas dataframe
ATL07dF, beamStr, beamStrength = ut.get_atl07_data_beam_extra(ATL07_dataFile, beamStr='', beamNum=int(beam_num), km_unit=True)
ATL10dF, _ = ut.get_atl10_data_beam_extra(ATL10_dataFile, beamStr, km_unit=True)
...
plt.colorbar()
plt.savefig(figPath+'s2-extra/overlay'+date+beamStr+relStr+str(Slim)+str(Nlim)+example+'.png', dpi=300)
...
crop.dump(figPath+'s2-extra/cropped_image'+date+beamStr+relStr+str(Slim)+str(Nlim)+example)
np.savetxt(figPath+'s2-extra/cropped_image'+date+beamStr+relStr+str(Slim)+str(Nlim)+example+'.txt', crop)
print(crop.shape)
...
solar_elev_mean=np.round(np.mean(sub_dF07['solar_elevation'].values), decimals=1)
solar_elev_mean
...
ax1.annotate(date+' '+relStr+' '+beamStr+' ('+beamStrength+')  N(ATL07) segments = '+str(atl07_segs)+' solar elev = '+str(solar_elev_mean),xy=(0.02, 1.05), xycoords='axes fraction')
...
#=========== Subplot 3 ===========
ax3 = plt.subplot2grid((16, 1), (4, 0), rowspan=2)
...
height_min = np.floor(np.nanmin(sub_dF07['height'].values)) 
height_max = np.ceil(np.nanmax(sub_dF07['height'].values))     

plt.scatter(sub_dF07['along_dist'],sub_dF07['height'], c=sub_dF07['height'], s=1, vmin=height_min, vmax=height_max, cmap='hot', zorder=2)
#plt.autoscale(enable=True, axis='x', tight=True)
ax3.xaxis.set_ticklabels([])
...
plt.ylim(height_min,height_max+1)
#plt.xlim(ax5.get_xlim())
plt.xlim([sub_dF07.along_dist.iloc[0], sub_dF07.along_dist.iloc[-1]])
...
ax3.annotate('N='+str(ssh07_segs)+' (ATL07 candidate ssh segs)',xy=(0.02, 0.63), xycoords='axes fraction')

#=========== Subplot 4 ===========
ax4 = plt.subplot2grid((16, 1), (6, 0), rowspan=2)
...
plt.ylabel('Photon Rate (ph/sh)', color='k', fontsize=12)
#plt.xlim(ax5.get_xlim())
plt.xlim([sub_dF07.along_dist.iloc[0], sub_dF07.along_dist.iloc[-1]])
...
#=========== Subplot 5 ===========
ax5 = plt.subplot2grid((16, 1), (8, 0), rowspan=2)
...
plt.ylabel('Norm Bgrnd Rate (MHz)', color='k', fontsize=12)
...
#=========== Subplot 6 ===========
ax6 = plt.subplot2grid((16, 1), (10, 0), rowspan=2)
...
#=========== Subplot 6 ===========
ax7 = plt.subplot2grid((16, 1), (12, 0), rowspan=2)
...
plt.scatter(sub_dF07['along_dist'],100.*(sub_dF07['segs_used']/sub_dF07['segs_total']), c='m', s=1, zorder=2)
#plt.autoscale(enable=True, axis='x', tight=True)
...
#=========== Subplot 7 ===========
ax8 = plt.subplot2grid((16, 1), (14, 0), rowspan=2)
            
#plt.scatter(sub_dF07['along_dist'],sub_dF07['cloud_flag_asr'], c=sub_dF07['layer_flag'], s=2, vmin=-0.1, vmax=1.1, cmap='RdBu', zorder=2)

im81 = plt.plot(sub_dF07['along_dist'],sub_dF07['cloud_flag_asr'], color='c', label='cloud_flag_asr', zorder=2)
im82 = plt.plot(sub_dF07['along_dist'],sub_dF07['cloud_flag_atm'], color='m', label='cloud_flag_atm', zorder=2)
...
for index, row in sub_dF07.iterrows():
    ...
    # layer flag
    if row['layer_flag'] > 0.5:
        plt.axvspan(x0,x1, facecolor='0.5', alpha=0.3)

atm_max = np.nanpercentile(sub_dF07['cloud_flag_atm'].values, 99)
asr_max = np.nanpercentile(sub_dF07['cloud_flag_asr'].values, 99)
max_cloud_flag = np.amax([atm_max, asr_max])

plt.ylim(-0.5,np.ceil(max_cloud_flag)+1.5)


leg = ax8.legend(loc=1, ncol=2,columnspacing=0.5, labelspacing=1, handletextpad=0.5, borderaxespad=0.05, frameon=False)

plt.ylabel('ASR/ATM cloud flag', color='k', fontsize=10)
ax8.tick_params(axis='y', colors='k', labelsize=12)
...
ax8.annotate(r'shaded by aggregate layer_flag = 1',xy=(0.01, 0.88), color='k',xycoords='axes fraction')
...
plt.subplots_adjust(left=0.08, right=0.91, top=0.96, bottom=0.05, wspace=0, hspace=0)
### Save plot to file
plt.savefig(figPath+'/S-2_07_09_overlay_'+date+'_beam'+beam_num+beamStr+relStr+str(Slim)+str(Nlim)+'_'+example+'.png', dpi=300)
