...
def get_geoTIFF_info(geoTIFF, plot=False):
  ...
  geoTIFF_inproj = int(gdal.Info(input, format='json')['coordinateSystem']['wkt'].rsplit('"EPSG","', 1)[-1].split('"')[0])
...
figPath='/Users/aapetty/GitHub-output/ICESat-2/plot_s2_comparison/'
...
# Print output
floe_lengthv1_mean=int(np.mean(pddata_v1.chord_lengths))
floe_lengthv1_std=int(np.std(pddata_v1.chord_lengths))

floe_lengthv2_mean=int(np.mean(pddata_v2.chord_lengths))
floe_lengthv2_std=int(np.std(pddata_v2.chord_lengths))
...
#for index, row in sub_dF07.iterrows():
    #x0 = row['along_dist'] - shadingSize/1000
    #x1 = row['along_dist'] + shadingSize/1000
    
    # cloud
    #if row['layer_flag'] > 0.5:
    #    plt.axvspan(x0,x1, facecolor='0.5', alpha=0.4)
...
for f1 in floe_groups07v1:
    #idx0=np.where(sub_dF07['height_id']==floe_groups[f][0])
    #idx1=np.where(sub_dF07['height_id']==floe_groups[f][-1])
    x0=f1[0]/1000.
    x1=f1[-1]/1000.
    #print(x0, x1)
    if (fi % 2) == 0: 
...
ax8.annotate(r'Chords',xy=(0.01, 0.88), color='r',xycoords='axes fraction')

ax8.annotate(floe_lengthv1_mean_str+r' km  (C$_l^{v1}$)',xy=(0.01, 0.56), color='r',xycoords='axes fraction')
ax8.annotate(floe_lengthv2_mean_str+r' km  (C$_l^{v2}$)',xy=(0.01, 0.41), color='m',xycoords='axes fraction')

ax8.annotate(lead_fraction_str+r' %  (L$_f^{v1}$)',xy=(0.01, 0.26), color='k',xycoords='axes fraction')
ax8.annotate(lead_fraction2_str+r' %  (L$_f^{v2}$)',xy=(0.01, 0.11), color='k',xycoords='axes fraction')
